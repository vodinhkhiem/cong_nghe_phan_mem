...
class FeedbackModel(Base):
    __tablename__ = 'feedbacks' # Hoặc 'peer_reviews'
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    reviewer_id = Column(Integer, ForeignKey('users.id'), nullable=False) # Người chấm
    target_id = Column(Integer, ForeignKey('users.id'), nullable=False)   # Người được chấm
    
    # Đánh giá cho Checkpoint nào hoặc Project nào
    checkpoint_id = Column(Integer, ForeignKey('checkpoints.id'), nullable=True)
    
    score = Column(Integer, nullable=False) # Điểm đánh giá (1-5 sao hoặc 1-10)
