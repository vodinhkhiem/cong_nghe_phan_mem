...
class CollabService:
    ...
    @staticmethod
    def save_whiteboard_snapshot(db: Session, team_id: int, json_data: str):
        """
        Lưu trạng thái bảng trắng (Snapshot).
        """
        workspace = CollabService.get_or_create_workspace(db, team_id)
        
        snapshot = db.query(WhiteboardSnapshotModel)\
                     .filter(WhiteboardSnapshotModel.workspace_id == workspace.id)\
                     .first()
                     
        if snapshot:
            snapshot.data = json_data # Update
        else:
            snapshot = WhiteboardSnapshotModel(workspace_id=workspace.id, data=json_data) # Insert
            db.add(snapshot)
    ...
    @staticmethod
    def get_whiteboard_data(db: Session, team_id: int):
        workspace = CollabService.get_or_create_workspace(db, team_id)
        snapshot = db.query(WhiteboardSnapshotModel)\
                     .filter(WhiteboardSnapshotModel.workspace_id == workspace.id)\
                     .first()
